...
exports.AppView = Backbone.View.extend
	...
	events:
		...
		'click #upload_snapshot_all': 'select_snapshot_all'
		'click #upload_snapshot_allunsel': 'select_snapshot_allunsel'
	...
	upload_snapshot:()->
		...
		if !$selected or $selected.length is 0
			...
			$selected.each ( idx, dom )->
				...
				snapshot.upload( {api_url: api_url} ).done( this.upload_done ).fail( this.upload_fail );
	...
	upload_done:( response )->
		...
		if rlt.ok
			...
			.done ( data )->
				if data.success
					selUploadedLength += 1
					layer.msg( """共要上传图片#{selLength}张，已成功上传#{selUploadedLength}！""", {icon: 1, time: 1000 } )
					$curDom.addClass('uploaded').data( 'id', data.obj.ID )
					$curDom.after( """<span class="sign">√</span>""" )
				else
					selFiledLength += 1
